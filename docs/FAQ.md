...
| dev | Runs tests, lints, starts dev webserver, and opens the app in your default browser. Does so "silently" without displaying various console messages. |
| lint:tools | Runs ESLint on build related JS files. (eslint-loader lints src files via webpack when `npm run dev` is run) |
